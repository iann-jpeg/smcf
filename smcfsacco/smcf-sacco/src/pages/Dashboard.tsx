      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              (s.recentTransactions as any[])?.map((txn: any) => (
                <div key={txn.id} className="flex items-center justify-between py-2 border-b last:border-0">
                  <div>
                    ...
                    {(txn.mpesa_ref || txn.mpesaRef) && (
                      <p className="text-[11px] text-muted-foreground">M-Pesa: {txn.mpesa_ref || txn.mpesaRef}</p>
                    )}
                  </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {isAdmin && (
              <button onClick={() => navigate("/admin-email")} className="w-full text-sm text-primary hover:underline text-center">
        ...
        <Card className="border-primary/20 bg-primary/5">
          ...
          <CardContent className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              Send broadcast emails to member and user emails, and review inbox activity.
            </p>
            <button
