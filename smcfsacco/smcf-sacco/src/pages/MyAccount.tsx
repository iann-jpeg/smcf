...
import { Link } from "react-router-dom"; // useSearchParams imported above
import { useMyMember, useMyLoans, useMyRepayments, useMyTransactions, useMySavingsHistory, useMyGuarantorRequests, useRespondToGuarantorRequest, useMyShareSummary } from "@/hooks/useMyAccount";
...
import { api, sendMemberMessage } from "@/lib/api";
...
export default function MyAccount() {
  ...
  const { data: shareSummary } = useMyShareSummary(member?.id);
  ...
  const [fallbackVerificationCode, setFallbackVerificationCode] = useState("");
  const [verifyingEmail, setVerifyingEmail] = useState(false);
  const [resendingVerification, setResendingVerification] = useState(false);

  const { data: authMe } = useQuery({
    queryKey: ["auth-me"],
    queryFn: async () => api.get<any>("/auth/me"),
  });
  ...
  const currentEmail = email ?? member?.email ?? String(authMe?.email || "");
  const accountEmail = String(authMe?.email || "").toLowerCase().trim();
  const isEmailVerified = Boolean(authMe?.isEmailVerified);

  const handleResendVerification = async () => {
    const targetEmail = String(currentEmail || accountEmail || "").trim().toLowerCase();
    if (!targetEmail) {
    ...
    try {
      const result = await api.post<any>("/auth/resend-verification-email", { email: targetEmail });
      const fallbackCode = String(result?.verificationToken || "").trim();
      if (fallbackCode) {
        setFallbackVerificationCode(fallbackCode);
        setVerificationCode(fallbackCode);
        toast.success("Email service is unavailable. Use the one-time code shown below.");
      } else {
        setFallbackVerificationCode("");
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      const baselineEmail = String(member?.email || accountEmail || "").trim().toLowerCase();
      const trimmedPhone = String(currentPhone || "").trim();

      const emailChanged = Boolean(normalizedEmail) && normalizedEmail !== baselineEmail;
      ...
      if (emailChanged) {
        const verifyResult = await api.put<any>("/auth/email/update-and-verify", { email: normalizedEmail });
        const fallbackCode = String(verifyResult?.verificationToken || "").trim();
        if (fallbackCode) {
          setFallbackVerificationCode(fallbackCode);
          setVerificationCode(fallbackCode);
          toast.success("Email updated. Use the one-time code below to verify.");
        } else {
          setFallbackVerificationCode("");
          toast.success("Email updated. A verification code has been sent to your inbox.");
        }
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 md:hidden">
        ...
        >
          ...
          <span className="text-xs font-medium">My Loans</span>
        </Button>
        <Button
          variant={activeTab === "repayments" ? "default" : "outline"}
          className="flex flex-col items-center gap-1.5 h-auto py-3"
          onClick={() => setSearchParams({ tab: "repayments" })}
        >
          <CalendarCheck className="h-5 w-5" />
          <span className="text-xs font-medium">Repayments</span>
        </Button>
        <Button
          variant={activeTab === "savings" ? "default" : "outline"}
          className="flex flex-col items-center gap-1.5 h-auto py-3"
          onClick={() => setSearchParams({ tab: "savings" })}
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          icon={Landmark}
          variant="accent"
          subtitle={shareSummary ? `${Number(shareSummary.unitsHeld ?? 0).toLocaleString()} units @ KES ${Number(shareSummary.unitPrice ?? 100).toLocaleString()}` : undefined}
        />
        <StatCard
          title="Savings"
          ...
          <CardContent className="py-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 text-sm">
              <div>
                ...
                <p className="font-semibold text-lg">{Number(shareSummary.unitsHeld ?? 0).toLocaleString()}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Estimated Worth</p>
                <p className="font-semibold text-lg">KES {Number(shareSummary.estimatedWorth ?? 0).toLocaleString()}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Purchased</p>
                <p className="font-semibold">KES {Number(shareSummary.purchasedTotal ?? 0).toLocaleString()}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Transfers In</p>
                <p className="font-semibold text-green-600">KES {Number(shareSummary.transferredIn ?? 0).toLocaleString()}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Transfers Out</p>
                <p className="font-semibold text-red-600">KES {Number(shareSummary.transferredOut ?? 0).toLocaleString()}</p>
              </div>
            </div>
            <p className="text-xs text-muted-foreground mt-3">
              Share holdings feed into loan trust-score signals together with savings consistency and repayment behavior.
            </p>
          </CardContent>
        </Card>
      )}
      ...
      <div ref={tabsContainerRef}>
        ...
        <TabsList className="flex h-auto flex-wrap gap-1 rounded-xl border border-border bg-card p-1.5 shadow-sm w-full">
          ...
          <TabsTrigger value="messages" className="rounded-lg px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-foreground/60 transition-all hover:text-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm flex-1 sm:flex-none">
            <Bell className="mr-1 h-3.5 w-3.5" />
            <span className="hidden sm:inline">Message Admin</span>
            <span className="sm:hidden">Msg</span>
          </TabsTrigger>
          <TabsTrigger value="profile" className="rounded-lg px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-foreground/60 transition-all hover:text-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm flex-1 sm:flex-none">
        ...
        <TabsContent value="messages">
          <Card>
            <CardHeader>
              <CardTitle className="font-heading text-lg">Message Main SMCF Admin</CardTitle>
              <CardDescription>
                Write directly to the main admin team. They will review your message in the admin inbox.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Button
                type="button"
                disabled={sendingAdminMessage || !adminMessageSubject.trim() || !adminMessageBody.trim()}
                onClick={async () => {
                  ...
                  try {
                    ...
                    toast.success("Message sent to Main SMCF admin");
                    ...
                    <Alert>
                      ...
                      <AlertDescription className="space-y-3 mt-2">
                        <p className="text-xs text-muted-foreground">
                          Accounts created before this verification rollout should verify their email once. If you change your email, you must verify the new address like during signup.
                        </p>
                          ...
                          <div className="space-y-2">
                            ...
                            {fallbackVerificationCode && (
                              <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs">
                                <p className="font-semibold">Fallback one-time code</p>
                                <p className="font-mono tracking-wide mt-1">{fallbackVerificationCode}</p>
                              </div>
                            )}
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                              <Input
                                ...
                                maxLength={12}
                              />
                              <Button type="button" onClick={handleVerifyEmail} disabled={verifyingEmail}>
