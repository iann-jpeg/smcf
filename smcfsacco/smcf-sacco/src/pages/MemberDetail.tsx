...
import { useMember, useUpdateMember, useLinkMemberAccount, useVerifyMemberKyc } from "@/hooks/useMembers";
...
import { ArrowLeft, Wallet, Landmark, TrendingUp, Pencil, Link2, Link2Off, UserCheck, FileText, Eye, Download, X, ShieldCheck } from "lucide-react";
...
export default function MemberDetail() {
  ...
  const canVerifyKyc = hasRole("admin") || hasRole("credit_officer");
  ...
  const verifyMemberKyc = useVerifyMemberKyc();
  ...
  const [editForm, setEditForm] = useState({
    memberId: "", name: "", email: "", phone: "", shares: "", savings: "", status: "active",
  });
  ...
  const handleEdit = async () => {
    if (!id || !editForm.name.trim() || !editForm.memberId.trim()) {
    ...
    try {
      await updateMember.mutateAsync({
        id,
        memberId: editForm.memberId.trim().toUpperCase(),
  ...
  const kycVerified = Boolean((member as any).kyc_verified);
  const kycVerifiedAt = (member as any).kyc_verified_at;

  const handleVerifyKyc = async () => {
    if (!id) return;
    try {
      await verifyMemberKyc.mutateAsync(id);
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Badge variant={kycVerified ? "default" : "outline"}>{kycVerified ? "KYC Verified" : "KYC Pending"}</Badge>
          </div>
          <p className="text-sm text-muted-foreground">{member.member_id} · {member.phone || "—"} · {member.email || "—"}</p>
        </div>
        {(isAdmin || canVerifyKyc) && (
          <div className="flex gap-2">
            {canVerifyKyc && !kycVerified && (
              <Button variant="default" size="sm" className="gap-1.5" onClick={handleVerifyKyc} disabled={verifyMemberKyc.isPending}>
                <ShieldCheck className="h-3.5 w-3.5" /> {verifyMemberKyc.isPending ? "Verifying..." : "Verify KYC"}
              </Button>
            )}
            {isAdmin && (
              <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                {([
                  { label: "KYC Status", value: kycVerified ? "Verified" : "Pending verification" },
                  { label: "KYC Verified At", value: kycVerifiedAt ? new Date(String(kycVerifiedAt)).toLocaleString() : null },
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-2">
            {[
              { label: "Member ID", id: "e-member-id", field: "memberId", type: "text", required: true },
