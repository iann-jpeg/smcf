...
export default function Auth() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const identifierValue = loginIdentifier.trim();
      ...
      if (!res.ok) {
        if (data.requiresEmailVerification) {
          ...
          setVerificationEmail(data.email || email || loginIdentifier);
          setShowVerificationModal(true);
          setTab("signup");
        } else {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoading(false);
      if (!res.ok) {
        const firstValidationError = Array.isArray(data?.errors) ? data.errors[0]?.msg : null;
        toast.error(firstValidationError || data?.message || "Signup failed");
        return;
      }

      const requiresVerification = Boolean(
        data?.requiresEmailVerification || data?.data?.requiresEmailVerification
      );
              ...
              <form onSubmit={handleLogin} className="space-y-4 mt-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    required
                    placeholder="Email or Member ID (e.g. MEM24001)"
                  />
                </div>
                <div className="space-y-2">
